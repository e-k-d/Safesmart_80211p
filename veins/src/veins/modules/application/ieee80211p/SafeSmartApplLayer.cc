...
void SafeSmartApplLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        /*mapInterval = par("mapInterval");
        spatInterval = par("spatInterval");
*/
        isParked = false;
        ...
        sendMapEvt = new cMessage("map evt", SEND_MAP_EVT);
        sendSpatEvt = new cMessage("spat evt", SEND_SPAT_EVT);
        sendSRMEvt = new cMessage("srm evt", SEND_SRM_EVT);
        sendSSMEvt = new cMessage("ssm evt", SEND_SSM_EVT);
...
simtime_t SafeSmartApplLayer::computeAsynchronousSendingTime(simtime_t interval, ChannelType chan)
{
...
void SafeSmartApplLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial)
{
    ...
    if (VehicleBSM* bsm = dynamic_cast<VehicleBSM*>(wsm)) {
        ...
        //EV << "MYIDNOPACOTE " << this->myId << std::endl;
        ...
        //EV << "WSA NOPACOTE " << this->myId << std::endl;
    }
    else {
        if (dataOnSch)
        ...
        //EV << "WSM NOPACOTE " << this->myId << std::endl;
    }
}

void SafeSmartApplLayer::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
{
    Enter_Method_Silent();
...
void SafeSmartApplLayer::handlePositionUpdate(cObject* obj)
{
...
void SafeSmartApplLayer::handleParkingUpdate(cObject* obj)
{
    isParked = mobility->getParkingState();
}

void SafeSmartApplLayer::handleLowerMsg(cMessage* msg)
{
...
void SafeSmartApplLayer::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind()) {
...
void SafeSmartApplLayer::finish()
{
    /*if (isEV > 0)
    {
...
SafeSmartApplLayer::~SafeSmartApplLayer()
{
...
void SafeSmartApplLayer::startService(Channel channel, int serviceId, std::string serviceDescription)
{
...
void SafeSmartApplLayer::stopService()
{
    cancelEvent(sendWSAEvt);
    currentOfferedServiceId = -1;
}

void SafeSmartApplLayer::sendDown(cMessage* msg)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDown(msg);
}

void SafeSmartApplLayer::sendDelayedDown(cMessage* msg, simtime_t delay)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDelayedDown(msg, delay);
}

void SafeSmartApplLayer::checkAndTrackPacket(cMessage* msg)
{
    if (dynamic_cast<VehicleBSM*>(msg)) {
