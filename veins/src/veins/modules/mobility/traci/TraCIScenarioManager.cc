...
{
    //EV << "THE LUKAS 2 " << vehicleId << std::endl;
...
double TraCIScenarioManager::safesmartSubscribe(std::string vehicleId)
{
    //subscribeToVehicleVariables(vehicleId);
#ifdef DEBUG
    ...
    uint8_t variable1 = VAR_SPEED;
    uint8_t variable2 = VAR_SPEED;
    //uint8_t variable2 = VAR_ROAD_ID;
    uint8_t variable3 = VAR_ANGLE;
    uint8_t variable4 = VAR_POSITION;
    //uint8_t variable5 = VAR_SIGNALS;
    //uint8_t variable6 = VAR_LENGTH;
    //uint8_t variable7 = VAR_HEIGHT;
    //uint8_t variable8 = VAR_WIDTH;
    uint8_t contextDomain = 0xa4; //vehicles
    double range = VEHICLE_PTHRESHOLD;


    //TraCIBuffer buf = connection->query(CMD_SUBSCRIBE_VEHICLE_VARIABLE, TraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2 << variable3 << variable4 << variable5 << variable6 << variable7 << variable8);
    TraCIBuffer buf = connection->query(CMD_SUBSCRIBE_VEHICLE_CONTEXT, TraCIBuffer() << beginTime << endTime << objectId << contextDomain << 0.0 << variableNumber << variable1 << variable3 << variable2 << variable4);
    TraCIBuffer buf2 = connection->query(CMD_SUBSCRIBE_VEHICLE_CONTEXT, TraCIBuffer() << beginTime << endTime << objectId << contextDomain << range << variableNumber << variable1 << variable3 << variable2 << variable4);
    //subscribeContext(junctionID, tc.CMD_GET_VEHICLE_VARIABLE, 42, [tc.VAR_SPEED, tc.VAR_WAITING_TIME])

    EV_posx = EV_posy = EV_speed = EV_angle = -1.0;
    lowestInterTime = 99999999.0;
    processSafeSmartSubcriptionResult(buf);
    lowestInterTime = 99999999.0;
    processSafeSmartSubcriptionResult(buf2);
    return lowestInterTime;
    //processSafeSmartSubcriptionResult(buf);
    //ASSERT(buf.eof());
}

void TraCIScenarioManager::processSafeSmartSubcriptionResult(TraCIBuffer& buf)
{
#ifdef DEBUG
    EV << "Safesmart Process Subscription" << std::endl;
#endif
    //return;
    ...
    else if (commandId_resp == RESPONSE_SUBSCRIBE_VEHICLE_CONTEXT)
    {
        processVehicleContextSubscription(objectId_resp, buf);
    }
    else {
        EV << "commandresp = " << commandId_resp << std::endl;
        throw cRuntimeError("Received unhandled subscription result");
    }
}

bool TraCIScenarioManager::inIntersection(double EV_posx, double EV_posy)
{
    if ( ((fabs(EV_posx - RSU0_PX) < RSU_PTHRESHOLD) && (fabs(EV_posy - RSU0_PY) < RSU_PTHRESHOLD)) ||
       ((fabs(EV_posx - RSU1_PX) < RSU_PTHRESHOLD) && (fabs(EV_posy - RSU1_PY) < RSU_PTHRESHOLD)) ||
       ((fabs(EV_posx - RSU2_PX) < RSU_PTHRESHOLD) && (fabs(EV_posy - RSU2_PY) < RSU_PTHRESHOLD)) ||
       ((fabs(EV_posx - RSU3_PX) < RSU_PTHRESHOLD) && (fabs(EV_posy - RSU3_PY) < RSU_PTHRESHOLD)))
            return true;
    else
            return false;
}

void TraCIScenarioManager::processVehicleContextSubscription(std::string objectId, TraCIBuffer& buf)
{
#ifdef DEBUG
    EV << "SafeSmart Vehicle Context Subscription" << std::endl;
#endif
    ...
    //double EV_posx, EV_posy, EV_speed, EV_angle = 0.0;
    double interX, interY, interEVTime, interVTime;


    uint8_t contextDomain;
    uint8_t variableNumber_resp;
    uint8_t variableCount;
    int objectsCount;

#ifdef DEBUG
    EV << "process Vehicle Context Subscription" << std::endl;
#endif

    buf >> contextDomain;
#ifdef DEBUG
    EV << "ContextDomain: " << (int)(contextDomain) << std::endl;
#endif

    buf >> variableCount;
#ifdef DEBUG
    EV << "variableCount: " << (int)(variableCount) << std::endl;
#endif

    buf >> objectsCount;
#ifdef DEBUG
    EV << "objectsCount: " << objectsCount << std::endl;
#endif
    ...
    for(uint8_t k = 0; k < objectsCount; k++)
    {
        buf >> objectId;
#ifdef DEBUG
        EV << "objectId " << (int)(k) << ":" << objectId << std::endl;
#endif
        for (uint8_t j = 0; j < variableCount; ++j) {
                uint8_t variable1_resp; //VAR ID
                buf >> variable1_resp;
                uint8_t isokay; //VAR STATUS
                buf >> isokay;
                if (isokay != RTYPE_OK) {
                    EV << "!= RTYPE_OK" << std::endl;
                    uint8_t varType;
                    buf >> varType;
                  ...
                  buf >> px;
                  buf >> py;
                  //EV << "EV_Speed=" << EV_speed << std::endl;
                  if (k == 0 && objectsCount == 1 && EV_posx <= 0.001 && EV_posy <= 0.001)
                  {
                      EV_posx = px;
                      EV_posy = py;
                      if (inIntersection(EV_posx, EV_posy))
                      {
                          safetyMetric = 0;
                      }
                      else
                  ...
                  {
                      //EV << "k1=" << (int)(k) << std::endl;
                      if (inIntersection(EV_posx, EV_posy) && k != (objectsCount-1)) //EV is the last one
                      {
                        //EV << "k2=" << (int)(k) << std::endl;
                        isOnIntersection = true;
                        dx = fabs (EV_posx - px);
                        dy = fabs (EV_posy - py);
                        distance = sqrt(dx*dx+dy*dy);//dx > dy ? dx : dy;
                        ...
                        //if (fabs(angle_traci - EV_angle) > 0.0001)
                        {
                            //EV << "k3=" << (int)(k) << std::endl;
                            double tempEV_angle = ((90.0 - EV_angle) + 360.0); //weird SUMO angle formula...
                            if (tempEV_angle < 0.0)
                                tempEV_angle += 360.0;
                            else
                            if (tempEV_angle >= 360.0)
                                tempEV_angle -= 360.0;

                            double tempV_angle = ((90.0 - angle_traci) + 360.0); //weird SUMO angle formula...
                            if (tempV_angle < 0.0)
                                tempV_angle += 360.0;
                            else
                            if (tempV_angle >= 360.0)
                                tempV_angle -= 360.0;

                            double rad_traci = tempV_angle*M_PI/180;
                            double rad_EV = tempEV_angle*M_PI/180;
                            double angleDiff = 180 - fabs(fabs(tempV_angle - tempEV_angle) - 180); //deg diff of angles between vehicles

                            interX = ((py - EV_posy) - (px * tan(rad_traci) - EV_posx * tan(rad_EV))) / (tan(rad_EV) - tan(rad_traci));
                            interY = ((px - EV_posx) - (py * 1/tan(rad_traci) - EV_posy * 1/tan(rad_EV))) / (1/tan(rad_EV) - 1/tan(rad_traci));

                            //if (interX <= 10000.0 && interY <= 10000.0) //valid intersection position
                            {
                                //EV << "k4=" << (int)(k) << std::endl;
                                double nextEV_posx, nextEV_posy, nextV_posx, nextV_posy;

                                nextEV_posx = EV_posx + (EV_speed/100.0 * cos(rad_EV));
                                nextV_posx = px + (speed/100.0 * cos(rad_EV));
                                nextEV_posy = EV_posy + (EV_speed/100.0 * sin(rad_EV));
                                nextV_posy = py + (speed/100.0 * sin(rad_EV));

                                /*//Old intersection-based calculation
                                 *
                                 * double vDist = sqrt ((px-interX)*(px-interX) + (py-interY)*(py-interY)); //dist of V to intersection pos
                                double EVDist = sqrt ((EV_posx-interX)*(EV_posx-interX) + (EV_posy-interY)*(EV_posy-interY)); //dist of EV to intersection pos

                                if (vDist <= 4.0)
                                    interVTime = 0.0;
                                else
                                if (fabs(nextV_posx - interX) < fabs(px - interX) || fabs(nextV_posy - interY) < fabs(py - interY) )
                                    interVTime = sqrt(fabs(px - interX)*fabs(px - interX) + fabs(py - interY)*fabs(py - interY))/speed;  //fabs(px - interX)/(speed * cos(rad_traci));
                                else
                                    interVTime = 9999999.0;
                                ...
                                if (fabs(nextEV_posx - interX) < fabs(EV_posx - interX) || fabs(nextEV_posy - interY) < fabs(EV_posy - interY) )
                                    interEVTime = sqrt(fabs(EV_posx - interX)*fabs(EV_posx - interX) + fabs(EV_posy - interY)*fabs(EV_posy - interY))/EV_speed;
                                else
                                    interEVTime = 9999999.0;



                                if (fabs(interEVTime - interVTime) < 4.0 && interEVTime < 1000.0 && interVTime < 1000.0)
                                {
                                    if (interEVTime < lowestInterTime)
                                    {
                                        //lowestInterTime = interEVTime;
                                    }
                                }*/

                                double collisionDx, collisionDy, collisionDist, collisionThreshold;
                                ...
                                if (EV_speed > 0.0 || speed > 0.0)
                                    for (int i = 0; i < 200; i++)
                                    {
                                        nextEV_posx = nextEV_posx + (EV_speed/10.0 * cos(rad_EV));
                                        nextV_posx = nextV_posx + (speed/10.0 * cos(rad_EV));
                                        nextEV_posy = nextEV_posy + (EV_speed/10.0 * sin(rad_EV));
                                        nextV_posy = nextV_posy + (speed/10.0 * sin(rad_EV));

                                        collisionDx = fabs(nextEV_posx - nextV_posx);
                                        collisionDy = fabs(nextEV_posy - nextV_posy);
                                        collisionDist = sqrt(collisionDx*collisionDx + collisionDy*collisionDy);

                                        if (i == 0)
                                        {
                                            if (collisionDist > distance)
                                            {
                                                EV << "NOK BEGIN" << std::endl;
                                                EV << "getting away from each other" << std::endl;
                                                EV << "K=" << (int)(k) << " lowestInterTime=" << lowestInterTime << " i=" << (double)(i) << " time=" << simTime() << std::endl;
                                                EV << "angleDiff=" << angleDiff << " threshold=" << collisionThreshold << std::endl;
                                                EV << " V(spd,ang) ("  << speed << ", " << tempV_angle << ") (x,y)(" << px << ", " << py << ")" << ")" << " (nx,ny)(" << nextV_posx << ", " << nextV_posy << ")" << std::endl;
                                                EV << "EV(spd,ang) ("  << EV_speed << ", " << tempEV_angle << ") (x,y)(" << EV_posx << ", " << EV_posy << ")" << " (nx,ny)(" << nextEV_posx << ", " << nextEV_posy << ")" << std::endl;
                                                EV << "Inter(x,y): (" << interX << ", " << interY << ") Time(V,EV)=(" << interVTime << ", " << interEVTime << ")" << std::endl << std::endl;
                                                EV << "collisiondist=" << collisionDist << " dist=" << distance << std::endl;
                                                EV << "NOK END" << std::endl;
                                                break; //vehicles are getting away from each other
                                            }
                                        }

                                        if (collisionDist < collisionThreshold)
                                        {
                                            if (lowestInterTime > (i*0.1))
                                            {
                                                lowestInterTime = i*0.1;
                                                EV << "OK BEGIN" << std::endl;
                                                EV << "K=" << (int)(k) << " lowestInterTime=" << lowestInterTime << " i=" << (double)(i) << " time=" << simTime() << std::endl;
                                                EV << "angleDiff=" << angleDiff << " threshold=" << collisionThreshold << std::endl;
                                                EV << " V(spd,ang) ("  << speed << ", " << tempV_angle << ") (x,y)(" << px << ", " << py << ")" << ")" << " (nx,ny)(" << nextV_posx << ", " << nextV_posy << ")" << std::endl;
                                                EV << "EV(spd,ang) ("  << EV_speed << ", " << tempEV_angle << ") (x,y)(" << EV_posx << ", " << EV_posy << ")" << " (nx,ny)(" << nextEV_posx << ", " << nextEV_posy << ")" << std::endl;
                               ...
                               /* EV << "K=" << (int)(k) << std::endl;
                                ...
                                EV << "Inter(x,y): (" << interX << ", " << interY << ") Time(V,EV)=(" << interVTime << ", " << interEVTime << ")" << std::endl << std::endl;
*/
                                /*EV << "k= " << (int)(k) << " Angle: " << angle_traci << " Speed: " << speed << " Position: (" << px << "," << py << ")" << std::endl;
                                EV << "EV: Angle: " << EV_angle << " EVSpeed: " << (double)(EV_speed) << " Position: (" << EV_posx << "," << EV_posy << ")" << std::endl;
                                EV << "Nextpos: EV(" << nextEV_posx << "," << nextEV_posy << ") V(" << nextV_posx << "," << nextV_posy << ")" << std::endl;
                                EV << "InterX:" << interX << " InterY:" << interY << std::endl;
                                EV << "interTimes(EV,V)=(" << interEVTime << "," << interVTime << ")" << std::endl;
                                EV << "tempEVANgle=" << tempEV_angle << " tempVAngle=" << tempV_angle << std::endl << std::endl;*/
                            }
...
#ifdef DEBUG
                        EV << "k= " << (int)(k) << "(" << px << "," << py << ") " << "distance " << distance << "dx=" << dx << " dy=" << dy << std::endl;
#endif
                        if (distance <= VEHICLE_PTHRESHOLD)
                            safetyMetric++;
#ifdef DEBUG
                        EV << "Is on junction" << std::endl;
#endif
                      }
                      else
                      {
#ifdef DEBUG
                          EV << "Is NOT on junction" << std::endl;
#endif
                          safetyMetric = -1;
                      }
                  }
/*#ifdef DEBUG
                  EV << "VAR_POSITION (" << px << "," << py << ")" << std::endl;
                  EV << "EV_POS (" << EV_posx << "," << EV_posy << ")" << std::endl;
#endif*/
                  numRead++;
              }
              else if (variable1_resp == VAR_SPEED) {
/*#ifdef DEBUG
                  EV << "VAR_SPEED" << std::endl;
#endif*/
                  uint8_t varType;
                  buf >> varType;
                  ASSERT(varType == TYPE_DOUBLE);
                  buf >> speed;
                  if (k == 0 && objectsCount == 1 && EV_speed <= 0.001)
                  {
                      EV_speed = speed;
                      //EV << "EV SPEED ATT " << EV_speed << std::endl;
                  }
                  numRead++;
              }
              else if (variable1_resp == VAR_ANGLE) {
/*#ifdef DEBUG
                  EV << "VAR_ANGLE" << std::endl;
#endif*/
                  ...
                  buf >> angle_traci;
                  numRead++;
                  if (k == 0 && objectsCount == 1 && EV_angle <= 0.001)
                  {
                      EV_angle = angle_traci;
                  }
              }
              else {
                  throw cRuntimeError("Received unhandled vehicle context subscription result %d", variable1_resp);
              }
            }
    }
   ...
   if (!mod) {
       ...
       /*for (uint8_t j = 0; j < variableCount; ++j) {
           ...
           else if (variable1_resp == RESPONSE_SUBSCRIBE_VEHICLE_CONTEXT)
           {
               EV << "processVehicleSubscription: RESPONSE_SUBSCRIBE_VEHICLE_CONTEXT" << std::endl;
           }
           else {
               throw cRuntimeError("Received unhandled vehicle context subscription result %d", variable1_resp);
           }
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf)
{
    ...
    //EV << "process Vehicle Subscription" << std::endl;
...
void TraCIScenarioManager::processSubcriptionResult(TraCIBuffer& buf)
{
    ...
    {
        EV << "processSubcriptionResult: RESPONSE_SUBSCRIBE_VEHICLE_CONTEXT" << std::endl;
        processVehicleContextSubscription(objectId_resp, buf);
    }
    else {
        throw cRuntimeError("Received unhandled subscription result");
